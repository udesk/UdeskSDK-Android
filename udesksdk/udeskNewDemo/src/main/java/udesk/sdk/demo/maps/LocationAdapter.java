...
public class LocationAdapter extends BaseAdapter {
    ...
    public LocationAdapter(Context context, List<PoiItem> datas) {
        this.datas = datas;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (datas != null && datas.size() > 0) {
            PoiItem poiInfo = datas.get(position);
            viewHolder.tv_poi_address.setText(poiInfo.getCityName()+poiInfo.getAdName()+poiInfo.getSnippet());
            viewHolder.tv_poi_name.setText(poiInfo.getTitle());
